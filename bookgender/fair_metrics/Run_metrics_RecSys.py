...
"""
    ...
    Args:
        ranked_lists (pandas.DataFrame): complete set of rankings to measure
        test_rates (panda.DataFrame): test set
        group (GroupInfo Object): contains group information from ranked lists
        original_relev(pandas.DataFrame): system given relevance score
...
class metric_analysis:
    ...
    def __init__(self, ranked_lists, test_rates, group, original_relev=None, IAA=True, EE=True, AWRF=True, DRR=True, FAIR=True):
    ...
    def run_IAA(self, ranked_list, pweight=pos.geometric()):
        ...
        return pd.Series({'IAA': bg.unfairness(ranked_list, self.original_relev, self.group, weight_vector)})
    ...
    def run_default_setting(self, listsize=100):
